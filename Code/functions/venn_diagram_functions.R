...
calculateOverlaps <- function(gene_lists, list_names, gene_directions = NULL) {
  # Function to calculate overlaps between sets
  # gene_directions: named list of dataframes with columns 'gene' and 'direction' ('+' or '-')
  ...
  # Intersections between sets - WITH DIRECTION CHECKING
  if (length(gene_lists) >= 2) {
    for (i in 2:length(gene_lists)) {
      ...
      for (combo in combos) {
        ...
        # CRITICAL FIX: Filter intersection to keep only genes with SAME direction
        if (!is.null(gene_directions) && length(intersection) > 0) {
          # Check direction consistency for each gene in intersection
          valid_genes <- character(0)
          
          for (gene in intersection) {
            # Get direction of this gene in each set of the combo
            directions <- character(length(combo))
            all_found <- TRUE
            
            for (k in 1:length(combo)) {
              set_idx <- combo[k]
              set_name <- list_names[set_idx]
              
              # Find gene direction in this set
              if (!is.null(gene_directions[[set_name]])) {
                gene_row <- gene_directions[[set_name]][gene_directions[[set_name]]$gene == gene, ]
                if (nrow(gene_row) > 0) {
                  directions[k] <- gene_row$direction[1]
                } else {
                  all_found <- FALSE
                  break
                }
            ...
            # Only keep gene if it has the same direction in ALL sets
            if (all_found && length(unique(directions)) == 1) {
              valid_genes <- c(valid_genes, gene)
            }
          }
          
          intersection <- valid_genes
          message(paste("Filtered intersection for", paste(name_parts, collapse = " & "), 
                        "- kept", length(valid_genes), "genes with consistent direction"))
        }
...
processVennGeneration <- function(table_storage, selected_tables, filter_params, colors) {
  ...
  tryCatch({
    ...
    gene_directions <- list()  # Store direction info
    ...
    for (i in 1:length(selected_tables)) {
      ...
      # CRITICAL: Store direction for each gene
      gene_table <- table_storage[[table_name]]$data
      gene_table_subset <- gene_table[rownames(gene_table) %in% gene_lists[[i]], ]
      
      gene_directions[[list_names[i]]] <- data.frame(
        gene = rownames(gene_table_subset),
        direction = ifelse(gene_table_subset$avg_log2FC > 0, "+", "-"),
        stringsAsFactors = FALSE
      )
    }
    
    # DEBUG: Print gene counts
    message("=== VENN DIAGRAM DEBUG ===")
    for (i in 1:length(gene_lists)) {
      message(paste(list_names[i], ":", length(gene_lists[[i]]), "genes"))
      if (!is.null(gene_directions[[list_names[i]]])) {
        n_up <- sum(gene_directions[[list_names[i]]]$direction == "+")
        n_down <- sum(gene_directions[[list_names[i]]]$direction == "-")
        message(paste("  UP:", n_up, ", DOWN:", n_down))
      }
    }
    ...
    # Calculate all overlaps with direction checking
    overlaps <- calculateOverlaps(gene_lists, list_names, gene_directions)
