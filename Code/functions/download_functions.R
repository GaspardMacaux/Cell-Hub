...
downloadPlot <- function(plot_object, object_name, plot_name, file_type = "pdf", 
  ...
  #   file_type: "pdf", "png", "tiff", "jpeg", "svg"
  #   width: Plot width in inches
  #   height: Plot height in inches
  #   dpi: DPI for raster files (PNG, TIFF, JPEG)
  # Returns:
  #   List with filename and content function
  
  if (is.null(plot_object)) stop("plot_object is NULL")
  
  tryCatch({
    # Generate file name
    filename <- generateFileName(object_name, plot_name, file_type)
    
    # Map file extensions to proper ggsave device names
    device_mapping <- list(
      "pdf" = "pdf",
      "png" = "png", 
      "tiff" = "tiff",
      "tif" = "tiff",    # Handle both extensions
      "jpeg" = "jpeg",
      "jpg" = "jpeg",    # Handle both extensions
      "svg" = "svg"
    )
    
    # Get proper device name
    device_name <- device_mapping[[tolower(file_type)]]
    if (is.null(device_name)) {
      stop(paste("Unsupported file format:", file_type))
    }
    
    # Create content function with explicit device specification
    content <- function(file) {
      # Ensure proper parameters based on device type
      if (device_name %in% c("png", "tiff", "jpeg")) {
        ggsave(file, plot = plot_object, device = device_name, 
               width = width, height = height, dpi = dpi, units = "in")
      } else if (device_name == "pdf") {
        ggsave(file, plot = plot_object, device = device_name, 
               width = width, height = height, units = "in")
      } else if (device_name == "svg") {
        ggsave(file, plot = plot_object, device = device_name, 
               width = width, height = height, units = "in")
      }
    }
    ...
    content_type <- switch(device_name,
                           "pdf" = "application/pdf",
                           "png" = "image/png",
                           "tiff" = "image/tiff",
                           "jpeg" = "image/jpeg",
...
downloadCSV <- function(data_object, object_name, data_name = "data") {
  ...
  if (is.null(data_object)) stop("data_object is NULL")
...
downloadOpenDocumentSpreadsheet <- function(data_lists, object_name, data_name = "gene_lists") {
  ...
  if (is.null(data_lists) || length(data_lists) == 0) stop("data_lists is NULL or empty")
...
downloadSeuratObject <- function(seurat_object, object_name, show_modal = TRUE) {
  ...
  if (is.null(seurat_object)) stop("seurat_object is NULL")
    ...
    content = function(file) {
      tryCatch({
        ...
        if (download_type == "plot") {
          ...
          # Map file type to proper device name
          device_name <- switch(tolower(file_type),
                                "pdf" = "pdf",
                                "png" = "png", 
                                "tiff" = "tiff",
                                "tif" = "tiff",
          ...
          if (inherits(data, "gList")) {
            ...
            # For regular ggplots, use ggsave with corrected device name
            if (device_name %in% c("png", "tiff", "jpeg")) {
              ggsave(file, plot = data, device = device_name, 
                     width = width, height = height, dpi = dpi, units = "in")
            } else {
              ggsave(file, plot = data, device = device_name, 
                     width = width, height = height, units = "in")
        ...
        message("ERROR in download content function: ", e$message)
